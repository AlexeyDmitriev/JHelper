...
public class TaskConfigurationTargetProvider extends ExecutionTargetProvider {
	...
	public @NotNull List<ExecutionTarget> getTargets(
			...
			@NotNull RunnerAndConfigurationSettings runnerAndConfigurationSettings
	) {
		if (!(runnerAndConfigurationSettings.getConfiguration() instanceof TaskConfiguration)) {
		...
		List<ExecutionTarget> runnerTargets = ExecutionTargetManager.getInstance(project).getTargetsFor(testRunner);
